...
void	port_init(void);
port_server_ref_t *	port_create(
	    const char *name, size_t namelen, int *error);
port_client_ref_t *	port_open(
	    const char *name, size_t namelen, int *error);
...
port_request_t *	port_receive(
	    port_server_ref_t *ref, void *buf, size_t buflen);
void	port_response(
	    port_request_t *req, void *retval,
	    size_t retval_size, int *error);
