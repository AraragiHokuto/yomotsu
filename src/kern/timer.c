...
timer_heap_left_child(timer_heap_element_t *self)
{
	...
	return ret >= percpu()->timer_heap_end ?
		NULL : ret;
}
...
static void
timer_heap_insert(timer_deadline_t deadline,
		  timer_callback_t callback, void *data)
{
	...
	ASSERT(percpu()->timer_heap_end - percpu()->timer_heap
	       < percpu()->timer_heap_size);
...
timer_heap_extract(void)
{
	...
	if (timer_heap_top() == percpu()->timer_heap_end) {
		return;
	}

	kmemcpy(timer_heap_top(), percpu()->timer_heap_end,
...
void
timer_set_timeout(timer_uduration_t timeout,
		  timer_callback_t callback, void *data)
{
	timer_heap_insert(timer_get_timestamp() + timeout,
			  callback, data);
}
