...
get_pml4e(address_space_t *as, uint pml4_idx) {
	pml4e_t *pml4	= as ?
		vm_get_vma_for_pma(as->pml4_paddr)
...
{
	u64 extended	= ((u64)address & (1ull << 47) ?
			   ~(0ull) : 0);
	return (void*)((extended & 0xffff000000000000)
		       | ((u64)address & 0xffffffffffff));
}

static void *
get_page(u64 pml4_idx, u64 pdp_idx, u64 pd_idx) {
	return sign_extend((void*)((pml4_idx << 39)
				   | (pdp_idx << 30)
				   | (pd_idx << 21)));
}
...
vm_get_pma_by_vma(void *pml4_vaddr, void *vma)
{
	...
	return (pde && pde->present) ?
		((pde->addr << 12) + VM_OFFSET_PAGE_2M((uintptr)vma)): 0;
}
...
void *
vm_alloc_vaddr(address_space_t *as,
	       void *vaddr_begin, void *vaddr_end, size_t size)
{
        byte *p	= vaddr_begin;
...
#define RECURSIVE_PDP(va)	((pdpe_t*)(0xffff804020000000 |		\
					   (PML4IDX(va) << 12)))
#define RECURSIVE_PD(va)	((pde_t*)(0xffff804000000000 |		\
					  ((PML4IDX(va) << 21) |	\
					   (PDPIDX(va) << 12))))
...
vm_kern_map_page(void *vma, uintptr pma)
{
	...
	void *ret	= vm_map_page(NULL, vma, pma,
				      VM_PAGE_WRITABLE
				      | VM_PAGE_EXECUTABLE
				      | VM_PAGE_PRIVILEGED
				      | VM_PAGE_GLOBAL);
...
vm_kern_alloc_anywhere(size_t size)
{
	...
	mutex_acquire(&vm_kern_table_lock);
	byte *vma	= vm_alloc_vaddr(
                NULL, KERNEL_VADDR_BEGIN, KERNEL_VADDR_END, size);
	if (!vma) {
		mutex_release(&vm_kern_table_lock);
		return NULL;
	}
...
kmem_heap_init(void)
{
	...
	kheap_chunk_t *first_chunk = vm_alloc_vaddr(
        ...
        VERIFY(vm_kern_map_page(first_chunk, first_chunk_pm),
...
{
	VERIFY(chunk->magic == KHEAP_MAGIC_FREE
...
kmem_alloc(size_t size)
{
	...
	if (i == &chunk_list_head) {
		...
		p = CONTAINER_OF(chunk_list_head.prev,
...
kmem_free(void *ptr)
{
	...
	kheap_aligned_header_t	*alignh	= (void*)((byte*)ptr
	    - sizeof(kheap_aligned_header_t));
	...
	kheap_chunk_t	*prev	= chunk->chunk_list_node.prev
		== &chunk_list_head ? NULL
		: CONTAINER_OF(chunk->chunk_list_node.prev,
			       kheap_chunk_t, chunk_list_node);
	kheap_chunk_t	*next	= chunk->chunk_list_node.next
		== &chunk_list_head ? NULL
		: CONTAINER_OF(chunk->chunk_list_node.next,
			       kheap_chunk_t, chunk_list_node);
