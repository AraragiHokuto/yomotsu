...
{
	kprintf("ISR handler called: vector = %d, errcode = %d,"
		" return_rip = 0x%x\n",
	...
	case 14:
		...
		break;
	default:
		PANIC("ISR handler not implemented: %d %s",
		      vector, isr_shortname[vector]);
		break;
	}
}
...
find_lapic_base(void)
{
	...
	for (acpi_madt_record_t *r = acpi_madt_first_record();
	     r; r = acpi_madt_next_record(r)) {
		if (r->header.type != 5) continue;
		kprintf("\t64bit APIC base addr override: 0x%x\n",
...
{
	PERCPU.defer_queue	= kmem_alloc(
		DEFER_QUEUE_INIT_SIZE * sizeof(irq_defer_queue_t));
...
{
	PERCPU.irq_handlers	= kmem_alloc(
		sizeof(irq_handler_t) * (IDT_GATES - IRQ_OFFSET));
	VERIFY(PERCPU.irq_handlers,
	       "OOM in interrupt_init_bsp(): irq_handlers");
	...
	apic_reg_base	= vm_get_vma_for_pma(apic_base);
        if (!vm_is_mapped(NULL, apic_reg_base)) {
                vm_map_page(NULL, apic_reg_base, apic_base,
                        VM_PAGE_WRITABLE
                        | VM_PAGE_PRIVILEGED
                        | VM_PAGE_GLOBAL);
        }
