...
{
	size_t hash	= kstr_hash(
		ifce->name, kstrlen(PORT_NAME_MAX_LEN, ifce->name));

	size_t i	= hash % PORT_IFCE_HASH_SIZE;
	do {
		void *expected	= NULL;
		if (atomic_cmpxchg_strong_ptr(
			    port_ifce_hash[i], expected, ifce,
...
{
	size_t hash	= kstr_hash(
		name, kstrlen(PORT_NAME_MAX_LEN, name));

	size_t i	= hash % PORT_IFCE_HASH_SIZE;
	do {
		port_ifce_t *ret	= atomic_load_ptr(
			port_ifce_hash[i], __ATOMIC_ACQUIRE);

		if (!ret) {
			return NULL;
		}

		if (kstrequ(PORT_NAME_MAX_LEN, ret->name, name)) {
			return ret;
		}
...
u64
port_ifce_debug_print_request(void *data, size_t datalen,
