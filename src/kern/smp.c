...
static struct {
	...
	.limit = sizeof(flat_gdt),
	.base = 0x7000 - sizeof(flat_gdt)
};
...
smp_start_ap(u32 target_lapic_id)
{
	...
	kmemcpy((void*)(0x7000 - sizeof(flat_gdt) - sizeof(flat_gdt_ptr)),
		&flat_gdt_ptr, sizeof(flat_gdt_ptr));
	kmemcpy((void*)(0x7000) - sizeof(flat_gdt),
		flat_gdt, sizeof(flat_gdt));
	...
	kmemcpy((void*)0x7000, &_ap_trampoline,
...
smp_start_all_aps(void)
{
	...
	for (acpi_madt_record_t *r = acpi_madt_first_record();
		...
		kprintf("\tSMP: processor found: Processor ID = %d,"
			" LAPIC ID = %d\n", r->lapic.processor_id,
			r->lapic.apic_id);
