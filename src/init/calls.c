...
extern long __do_syscall(
    u64 no, u64 arg1, u64 arg2, u64 arg3, u64 arg4, u64 arg5, u64 arg6);
...
long
mtransfer(
    kobject_handler_t das, kobject_handler_t sas, uintptr vaddr_d,
...
{
        return __do_syscall(
            SYSCALL_PORT_CREATE, (u64)name, namelen, 0, 0, 0, 0);
}

long
port_open(char *name, unsigned long namelen)
{
        return __do_syscall(SYSCALL_PORT_OPEN, (u64)name, namelen, 0, 0, 0, 0);
}
...
{
        return __do_syscall(
            SYSCALL_PORT_REQUEST, handler, (u64)req, 0, 0, 0, 0);
}
...
{
        return __do_syscall(
            SYSCALL_PORT_RECEIVE, handler, (u64)req, (u64)buf, buflen, 0, 0);
}

long
port_response(
    kobject_handler_t handler, u64 retval_small, void *retval,
    size_t retval_size)
{
        return __do_syscall(
            SYSCALL_PORT_RESPONSE, handler, retval_small, (u64)retval,
            retval_size, 0, 0);
}
