/* vfprintf.c -- vfprintf implementation */
/* this function is ported from musl */
#include <ctype.h>
#include <errno.h>
#include <float.h>
#include <inttypes.h>
...
static void
out(FILE *f, const char *s, size_t l)
{
        f->write(f, (const unsigned char *)s, l);
}

static void
pad(FILE *f, char c, int w, int l, int fl)
{
        char pad[256];
...
static char *
fmt_x(uintmax_t x, char *s, int lower)
{
...
/* static int */
/* fmt_fp(FILE *f, long double y, int w, int p, int fl, int t) */
/* { */
/*         uint32_t */
    ...
    FILE *f, const char *fmt, va_list *ap, union arg *nl_arg, int *nl_type)
{
        ...
        for (;;) {
                ...
                case 'm':
                        ...
                        case 's':
                                a = arg.p ? arg.p : "(null)";
                        z = (char *)memchr(a, '\0', p < 0 ? INT_MAX : p);
