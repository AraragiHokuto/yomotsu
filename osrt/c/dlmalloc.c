        ...
        for (;;) {
                ...
                if (stat == 0
                    && interlockedcompareexchange(
                           &malloc_global_mutex_status, (LONG)-1, (LONG)0)
                           == 0) {
                        InitializeCriticalSection(&malloc_global_mutex);
                        interlockedexchange(
                            &malloc_global_mutex_status, (LONG)1);
                        return;
                }
                SleepEx(0, FALSE);
        }
...
extern int pthread_mutexattr_setkind_np
    __P((pthread_mutexattr_t * __attr, int __kind));
...
#define compute_tree_index(S, I)                                                           \
        {                                                                                  \
                ...
                if (X == 0)                                                                \
                        ...
                        unsigned int K = (unsigned)sizeof(X) * __CHAR_BIT__                \
                                         - 1 - (unsigned)__builtin_clz(X);                 \
...
init_mparams(void)
{
        ...
        if (mparams.magic == 0) {
                ...
                gsize =
                    ((DEFAULT_GRANULARITY != 0) ? DEFAULT_GRANULARITY : psize);
#else  /* WIN32 */
                {
                ...
                /* Sanity-check configuration:
                   ...
                   Alignment, min chunk size, and page size must all be powers
                   of 2.
                */
...

                {
                        ...
                        magic &= ~(size_t)7U; /* improve chances of fault for
                                                 bad values */
                        /* Until memory modes commonly available, use
                         * volatile-write */
...
do_check_malloced_chunk(mstate m, void *mem, size_t s)
{
        if (mem != 0) {
                ...
                assert(sz >= s);
                /* unless mmapped, size is less than MIN_CHUNK_SIZE more than
                 * request */
        ...
        if (oldsize >= nb + SIZE_T_SIZE
            && (oldsize - nb) <= (mparams.granularity << 1))
                ...
                char *cp = (char *)CALL_MREMAP(
                    (char *)oldp - offset, oldmmsize, newmmsize, flags);
                if (cp != CMFAIL) {
        ...
        */
        ...
        if (MORECORE_CONTIGUOUS && !use_noncontiguous(m)) {
                ...
                if (ss == 0) { /* First time through or recovery */
                        ...
                        /* Subtract out existing available top space from
                         * MORECORE request. */
                        ssize = granularity_align(
                            nb - m->topsize + SYS_ALLOC_PADDING);
                        /* Use mem here only if it did continuously extend old
                         * space */
                ...
                if (tbase == CMFAIL) {      /* Cope with partial failure */
                        if (br != CMFAIL) { /* Try to use/extend the space we
                                               did get */
                                if (ssize < HALF_MAX_SIZE_T
                        ...
                        if (br != CMFAIL) { /* Use the space we did get */
                                ...
                                disable_contiguous(m); /* Don't try contiguous
                                                          path in the future */
                }
        ...
        if (tbase != CMFAIL) {
                ...
                if (!is_initialized(m)) { /* first-time initialization */
                        ...
                        if (is_global(m))
                                init_top(
                                    m, (mchunkptr)tbase, tsize - TOP_FOOT_SIZE);
                        else
#endif
                        {
                        ...
                        msegmentptr sp = &m->seg;
                        /* Only consider most recent segment if traversal
                         * suppressed */
                ...
                if (nb < m->topsize) { /* Allocate from new or extended top
                                          space */
...
release_unused_segments(mstate m)
{
        ...
        while (sp != 0) {
                ...
                if (is_mmapped_segment(sp) && !is_extern_segment(sp)) {
                        ...
                        /* Can unmap if first chunk holds entire segment and not
                         * pinned */
                        if (!is_inuse(p)
                            && (char *)p + psize
                                   >= base + size - TOP_FOOT_SIZE) {
                                tchunkptr tp = (tchunkptr)p;
...
sys_trim(mstate m, size_t pad)
{
        ...
        if (pad < MAX_REQUEST && is_initialized(m)) {
                pad +=
                    TOP_FOOT_SIZE; /* ensure enough room for segment overhead */

                if (m->topsize > pad) {
                        /* Shrink top space in granularity-size units, keeping
                         * at least one */
                                ...
                                if (is_mmapped_segment(sp)) {
                                        if (HAVE_MMAP && sp->size >= extra
                                            && !has_segment_link(
                                                ...
                                                size_t newsize =
                                                    sp->size - extra;
                                                (void)newsize; /* placate people
                                                                  compiling
                                                                  -Wunused-variable
                                                                */
                                                /* Prefer mremap, fall back to
                                                 * munmap */
                                                if ((CALL_MREMAP(
                                                         sp->base, sp->size,
                                                         newsize, 0)
                                        ...
                                        if (extra
                                            >= HALF_MAX_SIZE_T) /* Avoid
                                                                   wrapping
                                                                   negative */
                                                extra = (HALF_MAX_SIZE_T)
                                                        + SIZE_T_ONE - unit;
                                        ACQUIRE_MALLOC_GLOBAL_LOCK();
                                        {
                                                /* Make sure end of memory is
                                                 * where we last set it. */
                                                char *old_br =
                ...
                /* On failure, disable autotrim to avoid repeated failed future
                 * calls */
...
tmalloc_large(mstate m, size_t nb)
{
        ...
        if ((t = *treebin_at(m, idx)) != 0) {
                ...
                for (;;) {
                        ...
                        rt = t->child[1];
                        t  = t->child
                                [(sizebits >> (SIZE_T_BITSIZE - SIZE_T_ONE))
                                 & 1];
                        if (rt != 0 && rt != t) rst = rt;
                        if (t == 0) {
                                t = rst; /* set t to least subtree holding sizes
                                            > nb */
                                break;
                        }
                        sizebits <<= 1;
        ...
        /*
           ...
           If a small request (< 256 bytes minus per-chunk overhead):
             ...
             2. If it is big enough, use the dv chunk, which is normally the
                chunk adjacent to the one used for the most recent small
           request.
                ...
                if (bytes <= MAX_SMALL_REQUEST) {
                        ...
                        if ((smallbits & 0x3U)
                            != 0) { /* Remainderless fit to a smallbin. */
                                ...
                                if (smallbits != 0) { /* Use chunk in next
                                                         nonempty smallbin */
                                        mchunkptr b, p, r;
                                        size_t    rsize;
                                        bindex_t  i;
                                        ...
                                        /* Fit here cannot be remainderless if
                                         * 4byte sizes */
                                        if (SIZE_T_SIZE != 4
                                            && rsize < MIN_CHUNK_SIZE)
                                                set_inuse_and_pinuse(
                                                    gm, p, small_index2size(i));
                                        else {
                                                set_size_and_pinuse_of_inuse_chunk(
                                                    gm, p, nb);
                        ...
                        nb = MAX_SIZE_T; /* Too big to allocate. Force failure
                                            (in sys alloc) */
                else {
...
try_realloc_chunk(mstate m, mchunkptr p, size_t nb, int can_move)
{
        ...
        if (RTCHECK(
                ok_address(m, p) && ok_inuse(p) && ok_next(p, next)
                && ok_pinuse(next))) {
                if (is_mmapped(p)) {
        ...
        if (bytes >= MAX_REQUEST - alignment) {
                ...
                if (mem != 0) {
                        ...
                        if ((((size_t)(mem)) & (alignment - 1))
                            != 0) { /* misaligned */
                                /*
                                  Find an aligned spot inside chunk.  Since we
                                  need to give back leading space in a chunk of
                                  at least MIN_CHUNK_SIZE, if the first
                                  calculation places us at a spot with less than
                                  MIN_CHUNK_SIZE leader, we can move to the next
                                  aligned spot. We've allocated enough total
                                  room so that this is always possible.
                                */
                                char *    br       = (char *)mem2chunk((
                                    size_t)(((size_t)((char *)mem + alignment - SIZE_T_ONE)) & -alignment));
                                char *    pos      = ((size_t)(br - (char *)(p))
                                             >= MIN_CHUNK_SIZE)
                                                         ? br
                                                         : br + alignment;
                                ...
                                if (is_mmapped(p)) { /* For mmapped chunks, just
                                                        adjust offset */
                                        newp->prev_foot =
                                            p->prev_foot + leadsize;
                                        newp->head = newsize;
                                } else { /* Otherwise, give back leader, use the
                                            rest */
                        ...
                        if (!is_mmapped(p)) {
                                ...
                                if (size > nb + MIN_CHUNK_SIZE) {
                                        ...
                                        mchunkptr remainder =
                                            chunk_plus_offset(p, nb);
                                        set_inuse(m, p, nb);
                                        set_inuse(m, remainder, remainder_size);
                                        dispose_chunk(
                                            m, remainder, remainder_size);
                                }
                        }
...
ialloc(mstate m, size_t n_elements, size_t *sizes, int opts, void *chunks[])
{
        ...
        if (chunks != 0) {
                ...
                /* if empty req, must still return chunk representing empty
                 * array */
        ...
        */
        ...
        if (marray == 0) {
                ...
                set_size_and_pinuse_of_inuse_chunk(
                    m, array_chunk, array_chunk_size);
...
internal_inspect_all(
    mstate m,
    void (*handler)(
        void *start, void *end, size_t used_bytes, void *callback_arg),
    void *arg)
{
        if (is_initialized(m)) {
                mchunkptr   top = m->top;
                ...
                for (s = &m->seg; s != 0; s = s->next) {
                        ...
                        while (segment_holds(s, q)
                               && q->head != FENCEPOST_HEAD) {
                                ...
                                if (is_inuse(q)) {
                                        used =
                                            sz - CHUNK_OVERHEAD; /* must not be
                                                                    mmapped */
                                        start = chunk2mem(q);
                                } else {
                                        used = 0;
                                        if (is_small(
                                                sz)) { /* offset by possible
                                                          bookkeeping */
                                                start =
                                                    (void
                                                         *)((char *)q + sizeof(struct malloc_chunk));
                                        } else {
                                                start =
                                                    (void
                                                         *)((char *)q + sizeof(struct malloc_tree_chunk));
                                        }
                                }
                                if (start < (void *)next) /* skip if all space
                                                             is bookkeeping */
...
destroy_mspace(mspace msp)
{
        ...
        if (ok_magic(ms)) {
                ...
                while (sp != 0) {
                        ...
                        (void)base; /* placate people compiling
                                       -Wunused-variable */
                        sp = sp->next;
